...
open(F, "/root/tor-lab/tor/src/app/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/root/tor-lab/tor/src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/root/tor-lab/tor/doc/man/tor.1.txt") or die;
while (<F>) {
